...
network = NeuralNetwork(X_train_scaled.shape[1], 'regression', 'sigmoid', cost_function='R2', minibatches=minibatches, epochs = num_epochs, eta=learning_rate, lmbd=lmbd)         
...
cost = R2(test_pred, z_test)
print('Our neural network, R2: ' + str(cost))

####
plt.title('Fitting Franke function, sigmoid activation, R2 cost')
...
plt.legend() 
plt.yscale('log') 
plt.savefig('NNregr_franke_tuned_sigmoid_R2.pdf')
plt.show()
